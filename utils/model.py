...
def _prepare_row(
    ...
    row_df[numeric_cols] = arts["scaler"].transform(
        row_df[numeric_cols]
    )
...
def predict_transaction(
    ...
    Keys should match the Streamlit form fields:
        amount,
        hour,
        days_since_last,
        avg_amount_7d,
    ...
    # ---------------------------------------------------------
    prob = float(
        model.predict_proba(row_scaled.values)[0, 1]
    )
    ...
    #
    # IMPORTANT:
    # We intentionally use XGBoost's native pred_contribs
    # instead of shap.TreeExplainer.
    #
    # This avoids the compatibility problem between:
    #     XGBoost 3.x
    #     older SHAP versions
    #
    # XGBoost returns:
    #     [feature contributions..., bias]
    #
    # The final value is the bias/base contribution, so we
    # remove it with [:, :-1].
    # ---------------------------------------------------------
    ...
    top_features = [
        {
            ...
            "value": transaction.get(
                feat_names[i],
                row_scaled.iloc[0][feat_names[i]],
            ),
...
def predict_batch(df: pd.DataFrame) -> pd.DataFrame:
    """
    Score a DataFrame of transactions using the REAL trained model
    (vectorized — no per-row Python loop, so this scales to large
    uploaded CSVs).
    ...
    Missing columns are filled with safe defaults:
        0 for numeric/binary,
        the most common training category for categoricals.

    Missing columns are reported through the returned DataFrame's attrs.
    ...
    # ---------------------------------------------------------
    X[numeric_cols] = arts["scaler"].transform(
        X[numeric_cols]
    )
